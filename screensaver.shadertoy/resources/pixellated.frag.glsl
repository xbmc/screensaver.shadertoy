...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
  ...
  float vignetteAmt;
  vec2 uv = getuv_centerX(fragCoord, vec2(0.,0.), vec2(10.,10.), vignetteAmt);
  ...
  float pixelSize = 64. / iResolution.x * ubound.x;// pixels wide always.
  ...
  // plasma
  float a = plasma_a(plasma_uv, iGlobalTime, lbound, ubound);
