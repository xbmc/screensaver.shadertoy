...
float fbm( vec3 p )
{
    ...
    f += 0.2500*noise(p); p = mul(m,p)*2.0130;
    f += 0.1250*noise(p); p = mul(m,p)*2.0370;
...
void mainImage( out vec4 fragColor, in vec2 fragCoord ) {
  ...
  float as = 7.5;
  float pdir = nat(p, fbmz, 1.0) * 2.0 * pi * as;
  float d = as * 1.0 / length(iResolution);
  vec2 dp = vec2(d*sin(pdir), d * cos(pdir));
  vec2 q = p + dp;
  vec2 q2 = p - dp;
  float qdir = nat(q, fbmz, 1.0) * 2.0 * pi * as;
  pdir = nat(q2, fbmz, 1.0) * 2.0 * pi * as;
  vec2 c = (q2 + q) / 2.0;
